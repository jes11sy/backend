...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        external_services_task = asyncio.create_task(
            start_external_services_monitoring()
        )
        ...
        if background_tasks:
            try:
                ...
                logger.warning(
                    "Background tasks shutdown timeout - forcing termination"
                )
...
app.add_middleware(MetricsMiddleware, performance_collector=performance_collector)
...
app.add_middleware(
    RateLimitMiddleware, max_requests=settings.RATE_LIMIT_PER_MINUTE, window_seconds=60
)
