...
async def verify_webhook_security(
    ...
    if allowed_ips and client_ip not in allowed_ips:
        logging.warning(f"MANGO WEBHOOK: Blocked request from unauthorized IP: {client_ip}")
    ...
    if settings.MANGO_WEBHOOK_SECRET:
        ...
        expected_signature = x_signature  # TODO: Implement actual signature verification
...
async def mango_webhook(
    ...
    if call_state not in ["Disconnected", "Completed", "Finished"]:
        logging.warning(f"MANGO SKIP: Call state '{call_state}' not final, waiting for completion")
    ...
    if existing_by_phone:
        logging.warning(f"MANGO DUPLICATE BLOCKED BY PHONE: Phone {from_number}, existing request ID {existing_by_phone.id}, created at {existing_by_phone.created_at}")
        return {"ok": True, "detail": f"Заявка уже существует по телефону (ID: {existing_by_phone.id})"}
    ...
    try:
        ...
        if existing_call:
            logging.warning(f"MANGO DUPLICATE BLOCKED BY CALL_ID: CallID {call_id}, existing request ID {existing_call.id}")
            return {"ok": True, "detail": f"Заявка уже существует по call_id (ID: {existing_call.id})"}
    except Exception as e:
    ...
    try:
        ...
        logging.warning(f"MANGO REQUEST CREATED: Phone {from_number}, Type: {request_type_name}, ID: {new_request.id}, Campaign: {campaign.name}, CallID: {call_id}")
        
        return {"ok": True, "request_id": new_request.id, "type": request_type_name, "call_id": call_id}
    except Exception as e:
        logging.error(f"MANGO REQUEST CREATION ERROR: Phone {from_number}, CallID {call_id}, Error: {e}")
