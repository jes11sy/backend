...
async def get_database_stats(current_user: Administrator = Depends(require_admin)):
    """Получение статистики базы данных"""
    try:
        stats = await get_database_statistics()
        return {"status": "success", "data": stats, "timestamp": "2025-01-15T15:00:00Z"}
...
async def get_optimization_report(current_user: Administrator = Depends(require_admin)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to generate optimization report: {str(e)}"
        )


@router.post("/create-indexes")
async def create_indexes(current_user: Administrator = Depends(require_admin)):
...
async def refresh_views(current_user: Administrator = Depends(require_admin)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to refresh views: {str(e)}"
        )
...
async def cleanup_old_data(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to cleanup database: {str(e)}"
        )


@router.post("/analyze-performance")
async def analyze_performance(current_user: Administrator = Depends(require_admin)):
...
async def vacuum_analyze_tables(current_user: Administrator = Depends(require_admin)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to vacuum analyze: {str(e)}"
        )
    ...
    try:
        ...
        try:
            pool_status = {
                ...
                "utilization": getattr(pool, "checkedout", lambda: 0)()
                / max(getattr(pool, "size", lambda: 1)(), 1)
                * 100,
            }
        except Exception:
            pool_status = {
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get connection pool status: {str(e)}"
        )
...
async def get_slow_queries(
    ...
    min_time: float = Query(
        100.0, ge=0.0, description="Minimum query time in milliseconds"
    ),
    current_user: Administrator = Depends(require_admin),
):
...
async def get_index_usage(
    ...
    try:
        ...
        async with engine.begin() as conn:
            ...
            return {
                "status": "success",
                "data": {"index_usage": index_usage, "total_found": len(index_usage)},
                "timestamp": "2025-01-15T15:00:00Z",
            }
    except Exception as e:
...
async def get_table_sizes(current_user: Administrator = Depends(require_admin)):
    """Размеры таблиц"""
    try:
        from ..core.database import engine
        from sqlalchemy import text
        ...
        async with engine.begin() as conn:
            ...
            return {
                "status": "success",
                "data": {"table_sizes": table_sizes, "total_tables": len(table_sizes)},
                "timestamp": "2025-01-15T15:00:00Z",
            }
    except Exception as e:
        logger.error(f"Failed to get table sizes: {e}")
        raise HTTPException(
            status_code=500, detail=f"Failed to get table sizes: {str(e)}"
        )


@router.post("/optimize-full")
async def optimize_full(current_user: Administrator = Depends(require_admin)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to perform full optimization: {str(e)}"
        )
