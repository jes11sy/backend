...
async def check_file_access_permission(
    file_path: str, user: Union[Master, Employee, Administrator], db: AsyncSession
) -> bool:
    """
    ...
    file_record = await db.execute(select(File).where(File.file_path == file_path))
...
async def download_file(
    ...
    try:
        ...
        if not has_access:
            ...
            logging.warning(
                f"File access denied: {file_path} for user {current_user.login} from {client_ip}"
            )
            raise HTTPException(
