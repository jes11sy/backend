...
class BusinessRuleViolationError(BaseAppException):
    ...
    def __init__(
        self, message: str, rule: str, details: Optional[Dict[str, Any]] = None
    ):
...
def create_error_response(
    exception: BaseAppException, context: Optional[ErrorContext] = None
) -> JSONResponse:
...
def handle_database_error(error: Exception) -> BaseAppException:
    ...
    if "unique constraint" in error_str or "duplicate" in error_str:
        ...
        return DatabaseError(
            "Foreign key constraint violation", ErrorCode.FOREIGN_KEY_CONSTRAINT
        )
    elif "not found" in error_str:
        return RecordNotFoundError("Record", "unknown")
    elif "connection" in error_str:
        return DatabaseError(
            "Database connection error", ErrorCode.DATABASE_CONNECTION_ERROR
        )
    else:
...
def handle_validation_error(
    error: Exception, field: Optional[str] = None
) -> ValidationError:
    """Обработка ошибок валидации"""

    return ValidationError(message=str(error), field=field)
