...
class OptimizedRequestCRUD:
    ...
    async def find_duplicate_requests(
        db: AsyncSession, client_phone: str, time_window_minutes: int = 30
    ) -> List[Request]:
        """
    ...
    async def get_requests_by_phone_range(
        db: AsyncSession, client_phone: str, start_time: datetime, end_time: datetime
    ) -> List[Request]:
        """
    ...
    async def get_callcenter_statistics(
        ...
        city_stats = await db.execute(
            select(City.name, func.count(Request.id).label("count"))
            .select_from(base_query.join(City, Request.city_id == City.id).subquery())
            .group_by(City.name)
        )
...
class OptimizedUserCRUD:
    ...
    async def authenticate_user_optimized(
        ...
        # Проверяем мастеров
        master_query = (
            select(Master).options(joinedload(Master.city)).where(Master.login == login)
        )
    ...
    async def get_active_employees_by_role(
        db: AsyncSession, role_id: int, city_id: Optional[int] = None
    ) -> List[Employee]:
        """
        Получение активных сотрудников по роли
        Использует составной индекс idx_employees_role_city
        """
        filters = [Employee.role_id == role_id, Employee.status == "active"]
