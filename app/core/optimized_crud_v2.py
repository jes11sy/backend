...
class OptimizedCRUDv2:
    ...
    async def get_advertising_campaigns_cached(
        db: AsyncSession,
    ) -> List[Dict[str, Any]]:
    ...
    async def get_masters_by_city_cached(
    ...
    @staticmethod
    @cached(ttl=900, key_prefix="users")
    async def authenticate_user_cached(
    ...
    @staticmethod
    @performance_monitor
    async def get_request_by_id_optimized(
    ...
    async def get_callcenter_statistics_optimized(
        ...
        return {
            ...
            "conversion_rate": round(
                (
                    (status_stats.get("done", 0) / total_count * 100)
                    if total_count > 0
                    else 0
                ),
                2,
