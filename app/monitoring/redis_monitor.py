...
class RedisMonitor:
    ...
    def _register_metrics(self):
        ...
        redis_metrics = [
            MetricDefinition(
                "redis_connected",
                MetricType.GAUGE,
                "Redis подключение (1=connected, 0=disconnected)",
            ),
            MetricDefinition(
                "redis_uptime", MetricType.GAUGE, "Время работы Redis", "seconds"
            ),
            MetricDefinition(
                "redis_memory_used", MetricType.GAUGE, "Используемая память", "MB"
            ),
            MetricDefinition(
                "redis_memory_peak",
                MetricType.GAUGE,
            ...
            MetricDefinition(
                "redis_memory_usage", MetricType.GAUGE, "Использование памяти", "%"
            ),
            MetricDefinition(
            ...
            MetricDefinition(
                "redis_connections_active", MetricType.GAUGE, "Активные соединения"
            ),
            MetricDefinition(
                "redis_connections_blocked",
                MetricType.GAUGE,
                "Заблокированные соединения",
            ),
            MetricDefinition(
                "redis_keyspace_hits", MetricType.COUNTER, "Попадания в кеш"
            ),
            MetricDefinition(
                "redis_keyspace_misses", MetricType.COUNTER, "Промахи кеша"
            ),
            MetricDefinition(
                "redis_hit_rate", MetricType.GAUGE, "Коэффициент попаданий", "%"
            ),
            MetricDefinition(
                "redis_commands_processed", MetricType.COUNTER, "Обработанные команды"
            ),
            MetricDefinition(
                "redis_ops_per_sec", MetricType.GAUGE, "Операций в секунду"
            ),
            MetricDefinition(
                "redis_evicted_keys", MetricType.COUNTER, "Вытесненные ключи"
            ),
            MetricDefinition(
                "redis_expired_keys", MetricType.COUNTER, "Просроченные ключи"
            ),
            MetricDefinition(
                "redis_keyspace_size", MetricType.GAUGE, "Размер keyspace"
            ),
            MetricDefinition(
                "redis_slow_commands", MetricType.COUNTER, "Медленные команды"
            ),
            MetricDefinition(
                "redis_status",
                MetricType.GAUGE,
                "Статус Redis (0=healthy, 1=warning, 2=critical, 3=disconnected)",
            ),
        ]
    ...
    async def get_redis_metrics(self) -> RedisMetrics:
        ...
        if maxmemory > 0:
            ...
            memory_usage_percent = min(
                used_memory_mb / 512, 100
            )  # Предполагаем 512MB как базовый лимит
    ...
    def _record_metrics(self, metrics: RedisMetrics):
        ...
        metrics_collector.set_gauge(
            "redis_connections_total", metrics.total_connections
        )
        metrics_collector.set_gauge(
            "redis_connections_active", metrics.connected_clients
        )
        metrics_collector.set_gauge(
            "redis_connections_blocked", metrics.blocked_clients
        )
        metrics_collector.set_gauge("redis_hit_rate", metrics.hit_rate_percent)
        metrics_collector.set_gauge(
            "redis_ops_per_sec", metrics.instantaneous_ops_per_sec
        )
        ...
        if metrics.total_commands_processed > self.previous_stats.get(
            "total_commands_processed", 0
        ):
            metrics_collector.increment(
                "redis_commands_processed",
                metrics.total_commands_processed
                - self.previous_stats.get("total_commands_processed", 0),
            )
    ...
    def _get_recent_evictions(self) -> int:
        ...
        now = datetime.now()
        recent_metrics = [
            m for m in self.metrics_history if now - m.timestamp < timedelta(minutes=5)
        ]
    ...
    def get_redis_statistics(self) -> Dict[str, Any]:
        ...
        return {
            "uptime_hours": (
                recent_metrics[-1].uptime_seconds / 3600 if recent_metrics else 0
            ),
...
def record_cache_operation(operation: str, hit: bool = False, duration_ms: float = 0):
    ...
    if duration_ms > 0:
        metrics_collector.record(
            "redis_operation_duration", duration_ms, {"operation": operation}
        )
