...
class ServiceHealthChecker:
    ...
    async def check_recording_service(self) -> HealthCheck:
        ...
        try:
            ...
            task_active = (
                recording_service.task is not None and not recording_service.task.done()
            )
        ...
        try:
            ...
            return HealthCheck(
                ...
                details={
                    ...
                    "free_space_percent": (
                        free_percent if os.path.exists(media_dir) else 0
                    ),
                },
                duration_ms=duration,
                timestamp=datetime.now(),
...
class HealthMonitor:
    ...
    async def run_all_checks(self, use_cache: bool = True) -> Dict[str, HealthCheck]:
        ...
        if (
            ...
            and now - self.last_full_check < self.cache_duration
    ...
    async def get_health_summary(self, use_cache: bool = True) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "healthy": sum(
                    1 for c in checks.values() if c.status == HealthStatus.HEALTHY
                ),
                "degraded": sum(
                    1 for c in checks.values() if c.status == HealthStatus.DEGRADED
                ),
                "unhealthy": sum(
                    1 for c in checks.values() if c.status == HealthStatus.UNHEALTHY
                ),
                "unknown": sum(
                    1 for c in checks.values() if c.status == HealthStatus.UNKNOWN
                ),
            },
        }
