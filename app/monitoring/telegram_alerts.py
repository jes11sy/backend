...
async def send_telegram_alert(
    service: str, message: str, thread_id: Optional[int] = None
):
    """
    ...
    """
    if (
        not settings.TELEGRAM_ALERTS_ENABLED
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url, data=payload) as resp:
            if resp.status != 200:
                logger.error(
                    f"Telegram alert failed: {resp.status} {await resp.text()}"
                )
            else:
...
async def create_topic_and_alert(service: str, message: str):
    ...
    payload = {"chat_id": settings.TELEGRAM_CHAT_ID, "name": service.capitalize()}
    async with aiohttp.ClientSession() as session:
        async with session.post(url, data=payload) as resp:
            if resp.status == 200:
                ...
                logger.error(
                    f"Failed to create topic for {service}: {resp.status} {await resp.text()}\nОтправляю алерт в общий чат."
                )
