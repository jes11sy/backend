...
async def get_requests_optimized(
    db: AsyncSession, skip: int = 0, limit: int = 100, include_relations: bool = True
) -> List[Request]:
    """
...
async def get_request_optimized(
    db: AsyncSession, request_id: int, include_relations: bool = True
) -> Request | None:
    """
...
async def get_employees_optimized(
    db: AsyncSession, skip: int = 0, limit: int = 100, role_id: int | None = None
) -> List[Employee]:
    """
    Оптимизированная загрузка сотрудников
    """
    query = select(Employee).options(
        joinedload(Employee.role), joinedload(Employee.city)
    )
