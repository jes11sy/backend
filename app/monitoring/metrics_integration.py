        ...
        async def async_wrapper(*args, **kwargs):
            ...
            try:
                ...
                performance_collector.record_db_query(
                    f"{operation_name}_error", duration
                )

                # Увеличиваем счетчик ошибок
                metrics_collector.increment(
                    "db_errors_total", tags={"operation": operation_name}
                )
    ...
    def decorator(func: Callable):
        ...
        async def async_wrapper(*args, **kwargs):
            try:
                ...
                metrics_collector.increment(
                    f"business_event_{event_name}", value, tags={"status": "success"}
                )
        ...
        async def async_wrapper(*args, **kwargs):
            try:
                ...
                if result is not None:
                    ...
                    metrics_collector.increment(
                        "cache_operations", tags={"cache": cache_name, "result": "hit"}
                    )
                else:
                    performance_collector.record_cache_miss()
                    metrics_collector.increment(
                        "cache_operations", tags={"cache": cache_name, "result": "miss"}
                    )
...
class MetricsContext:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if self.start_time:
            ...
            metrics_collector.record(
                f"context_{self.context_name}_duration", duration, self.tags
            )
...
async def collect_request_lifecycle_metrics(
    request_id: str, user_id: Optional[str] = None, city_id: Optional[str] = None
):
    ...
    metrics_collector.record(
        "request_lifecycle_timestamp", time.time(), tags={**tags, "stage": "created"}
    )
...
async def collect_transaction_metrics(
    ...
    tags = {"transaction_id": transaction_id, "type": transaction_type}
...
async def collect_user_activity_metrics(
    user_id: str, activity_type: str, duration: Optional[float] = None
):
    """Сбор метрик активности пользователей"""
    tags = {"user_id": user_id, "activity": activity_type}
    ...
    metrics_collector.record("user_last_activity", time.time(), {"user_id": user_id})
