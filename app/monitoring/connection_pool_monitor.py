...
class ConnectionPoolMonitor:
    ...
    def _register_metrics(self):
        ...
        pool_metrics = [
            MetricDefinition(
                "db_pool_size", MetricType.GAUGE, "Размер пула соединений"
            ),
            MetricDefinition(
                "db_pool_checked_out", MetricType.GAUGE, "Активные соединения"
            ),
            MetricDefinition(
                "db_pool_overflow", MetricType.GAUGE, "Overflow соединения"
            ),
            MetricDefinition(
                "db_pool_invalid", MetricType.GAUGE, "Недействительные соединения"
            ),
            MetricDefinition(
                "db_pool_utilization", MetricType.GAUGE, "Использование пула", "%"
            ),
            MetricDefinition(
                "db_pool_wait_time",
            ...
            MetricDefinition(
                "db_connection_errors", MetricType.COUNTER, "Ошибки соединений"
            ),
            MetricDefinition(
                "db_connection_created", MetricType.COUNTER, "Созданные соединения"
            ),
            MetricDefinition(
                "db_connection_closed", MetricType.COUNTER, "Закрытые соединения"
            ),
            MetricDefinition(
                "db_slow_queries", MetricType.COUNTER, "Медленные запросы"
            ),
            MetricDefinition(
                "db_query_duration",
                MetricType.HISTOGRAM,
            ...
            MetricDefinition(
                "db_pool_status",
                MetricType.GAUGE,
                "Статус пула (0=healthy, 1=warning, 2=critical)",
            ),
        ]
    ...
    def get_pool_metrics(self) -> ConnectionPoolMetrics:
        ...
        status = self._determine_pool_status(
            utilization_percent, available_connections, invalid
        )
    ...
    def _determine_pool_status(
        self, utilization_percent: float, available_connections: int, invalid: int
    ) -> PoolStatus:
    ...
    def record_slow_query(
        self, query: str, duration_ms: float, connection_id: Optional[str] = None
    ):
    ...
    def record_connection_error(self, error: Exception):
        ...
        with self._lock:
            ...
            self.connection_events.append(
                {"event": "error", "timestamp": datetime.now(), "error": str(error)}
            )
    ...
    def _check_alerts(self, metrics: ConnectionPoolMetrics) -> List[Dict[str, Any]]:
        ...
        recent_slow_queries = len(
            [q for q in self.slow_queries if now - q.timestamp < timedelta(minutes=5)]
        )
...
def track_query_performance(
    query: str, duration_ms: float, connection_id: Optional[str] = None
):
