...
class MetricsCollector:
    ...
    def __init__(self, max_values: int = 1000):
        ...
        self._db_operation_semaphore = asyncio.Semaphore(
            5
        )  # Ограничиваем concurrent DB операции
    ...
    def record(
        ...
        metric_value = MetricValue(
            value=value, timestamp=datetime.utcnow(), tags=tags, metadata=metadata
        )
...
class BusinessMetricsCollector:
    ...
    def register_business_metrics(self):
        ...
        business_metrics = [
            MetricDefinition(
                "requests_total", MetricType.COUNTER, "Общее количество заявок"
            ),
            MetricDefinition(
                "requests_by_status",
                MetricType.COUNTER,
            ...
            MetricDefinition(
                "transactions_total", MetricType.COUNTER, "Общее количество транзакций"
            ),
            MetricDefinition(
                "transactions_amount", MetricType.GAUGE, "Сумма транзакций", "RUB"
            ),
            MetricDefinition("active_users", MetricType.GAUGE, "Активные пользователи"),
            MetricDefinition(
                "conversion_rate", MetricType.GAUGE, "Конверсия заявок", "%"
            ),
            MetricDefinition(
                "avg_processing_time",
                MetricType.GAUGE,
            ...
            MetricDefinition(
                "revenue_daily", MetricType.GAUGE, "Дневная выручка", "RUB"
            ),
            MetricDefinition(
                "calls_total", MetricType.COUNTER, "Общее количество звонков"
            ),
            MetricDefinition(
                "call_duration_avg",
                MetricType.GAUGE,
    ...
    async def collect_request_metrics(self, db: AsyncSession):
        ...
        async with self.metrics._db_operation_semaphore:
            try:
                        ...
                        if isinstance(value, dict):
                            for tag_key, tag_value in value.items():
                                self.metrics.set_gauge(
                                    ...
                                    (
                                        {"status": tag_key}
                                        if metric_name == "requests_by_status"
                                        else {"city": tag_key}
                                    ),
                                )
                        else:
                ...
                total_requests = await db.scalar(text("SELECT COUNT(*) FROM requests"))
                ...
                for status, count in status_counts:
                    self.metrics.set_gauge(
                        "requests_by_status", count, {"status": status}
                    )
        ...
        async with self.metrics._db_operation_semaphore:
            try:
                ...
                active_administrators = (
                    await db.scalar(
                        text(
                            "SELECT COUNT(*) FROM administrators WHERE status = 'active'"
                        )
                    )
                    or 0
                )
                ...
                total_active_users = (
                    active_masters + active_employees + active_administrators
                )
        ...
        async with self.metrics._db_operation_semaphore:
            try:
                ...
                total_calls = await db.scalar(
                    text(
                        "SELECT COUNT(*) FROM requests WHERE recording_file_path IS NOT NULL AND recording_file_path != ''"
                    )
                )
...
class PerformanceMetricsCollector:
    ...
    def register_performance_metrics(self):
        ...
        performance_metrics = [
            ...
            MetricDefinition(
                "db_queries_total", MetricType.COUNTER, "Количество DB запросов"
            ),
            MetricDefinition(
                "db_query_duration",
                MetricType.HISTOGRAM,
            ...
            MetricDefinition(
                "db_connections_active", MetricType.GAUGE, "Активные DB соединения"
            ),
            MetricDefinition(
                "memory_usage", MetricType.GAUGE, "Использование памяти", "MB"
            ),
...
async def start_metrics_collection():
    ...
    if not metrics_collector._running:
        ...
        metrics_collector._background_task = asyncio.create_task(
