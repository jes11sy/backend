...
redis_memory_usage_bytes = Gauge(
    "redis_memory_usage_bytes", "Redis memory usage in bytes", registry=registry
)
...
auth_success_rate = Gauge(
    "auth_success_rate", "Authentication success rate", registry=registry
)
...
file_storage_usage_bytes = Gauge(
    "file_storage_usage_bytes", "File storage usage in bytes", registry=registry
)
...
class MetricsCollector:
    ...
    def update_system_metrics(self):
        ...
        try:
            ...
            logger.debug(
                f"System metrics updated: CPU={cpu_percent}%, Memory={memory.percent}%, Disk={disk.percent}%"
            )
