...
PERFORMANCE_INDEXES_V2 = [
    ...
    Index(
        "idx_requests_phone_created_v2", Request.client_phone, Request.created_at.desc()
    ),
    ...
    Index(
        "idx_requests_phone_time_window_v2",
        Request.client_phone,
        Request.created_at.desc(),
    ),
    Index(
        "idx_requests_callcenter_report_v2",
        Request.city_id,
        Request.status,
        Request.created_at.desc(),
    ),
    Index(
        "idx_requests_master_assignment_v2",
        Request.city_id,
        Request.status,
        Request.master_id,
    ...
    Index("idx_requests_phone_datetime_v2", Request.client_phone, Request.created_at),
    # Индекс для статистики конверсии
    Index(
        "idx_requests_conversion_stats_v2",
        Request.city_id,
        Request.status,
        Request.created_at.desc(),
    ...
    Index(
        "idx_transactions_city_date_v2",
        Transaction.city_id,
        Transaction.specified_date.desc(),
    ),
    Index(
        "idx_transactions_type_date_v2",
        Transaction.transaction_type_id,
        Transaction.specified_date.desc(),
    ),
    Index(
        "idx_transactions_city_type_v2",
        Transaction.city_id,
        Transaction.transaction_type_id,
    ),
    ...
    Index(
        "idx_transactions_financial_report_v2",
        Transaction.city_id,
        Transaction.specified_date.desc(),
        Transaction.amount,
    ),
    ...
    Index(
        "idx_transactions_amount_range_v2",
        Transaction.city_id,
        Transaction.amount,
        Transaction.specified_date.desc(),
    ),
...
SPECIALIZED_INDEXES = [
    ...
    Index(
        "idx_requests_duplicate_check_v2",
        Request.client_phone,
        Request.status,
        Request.created_at.desc(),
    ...
    Index(
        "idx_requests_mango_integration_v2",
        Request.client_phone,
        Request.ats_number,
        Request.created_at.desc(),
    ),
    ...
    Index(
        "idx_requests_performance_report_v2",
        Request.city_id,
        Request.master_id,
        Request.status,
        Request.created_at.desc(),
...
FUNCTIONAL_INDEXES = [
    ...
    Index(
        "idx_requests_phone_partial_v2",
        text("substring(client_phone, 1, 10)"),
        Request.created_at.desc(),
    ),
    # Индекс для поиска по дате без времени
    Index("idx_requests_date_only_v2", text("DATE(created_at)"), Request.city_id),
    # Индекс для поиска по месяцу и году
    Index(
        "idx_transactions_month_year_v2",
        ...
        try:
            ...
            for index in PERFORMANCE_INDEXES_V2:
                try:
                    await conn.execute(
                        text(
                            f"CREATE INDEX IF NOT EXISTS {index.name} ON {index.table.name} {index._create_index_sql()}"
                        )
                    )
            ...
            for index in SPECIALIZED_INDEXES:
                try:
                    ...
                    if "already exists" not in str(e):
                        logger.warning(
                            f"Ошибка создания специализированного индекса {index.name}: {e}"
                        )
    ...
    async with engine.begin() as conn:
        try:
            ...
            for row in slow_queries.fetchall():
                logger.info(
                    f"Запрос: {row[0][:100]}... | Вызовы: {row[1]} | Среднее время: {row[3]:.2f}мс | Hit%: {row[7]:.1f}%"
                )
            ...
            for row in index_usage.fetchall():
                logger.info(
                    f"Таблица: {row[1]} | Индекс: {row[2]} | Сканирований: {row[3]} | Размер: {row[6]}"
                )

            # Анализируем неиспользуемые индексы
            unused_indexes = await conn.execute(
                text(
                    """
                SELECT 
                    schemaname,
            ...
            for row in table_sizes.fetchall():
                logger.info(
                    f"Таблица: {row[0]} | Общий размер: {row[1]} | Таблица: {row[2]} | Индексы: {row[3]}"
                )
