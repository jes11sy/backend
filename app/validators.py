...
def validate_text_length(
    text: str, field_name: str, min_length: int = 0, max_length: int = 1000
) -> str:
...
def validate_status(status_value: str, allowed_statuses: List[str]) -> str:
    ...
    if status_value not in allowed_statuses:
        raise ValidationError(
            f"Invalid status. Allowed values: {', '.join(allowed_statuses)}", "status"
        )
...
class RequestValidator:
    ...
    ALLOWED_STATUSES = ["new", "in_progress", "completed", "cancelled", "on_hold"]
    ...
    def validate_request_data(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "status" in data:
            validated_data["status"] = validate_status(
                data["status"], RequestValidator.ALLOWED_STATUSES
            )
    ...
    def validate_transaction_data(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "specified_date" in data:
            try:
                ...
                raise ValidationError(
                    "Invalid date format. Use YYYY-MM-DD", "specified_date"
                )
...
class UserValidator:
    ...
    def validate_user_data(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Валидация логина
        if "login" in data:
            login = validate_text_length(
                data["login"], "login", min_length=3, max_length=50
            )
