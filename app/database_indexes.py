...
DATABASE_INDEXES = [
    ...
    Index(
        "idx_requests_created_at", Request.created_at.desc()
    ...
    Index(
        "idx_requests_request_type_id", Request.request_type_id
    ),  # Фильтрация по типу
    Index(
        "idx_requests_advertising_campaign_id", Request.advertising_campaign_id
    ),  # Фильтрация по РК
    # Составные индексы для частых комбинаций
    Index(
        "idx_requests_phone_created", Request.client_phone, Request.created_at.desc()
    ),  # Поиск дубликатов
    Index(
        "idx_requests_city_status", Request.city_id, Request.status
    ),  # Отчеты по городам
    Index(
        "idx_requests_city_created", Request.city_id, Request.created_at.desc()
    ),  # Отчеты по городам с датой
    Index(
        "idx_requests_status_created", Request.status, Request.created_at.desc()
    ),  # Отчеты по статусам
    Index(
        "idx_requests_master_status", Request.master_id, Request.status
    ),  # Заявки мастера по статусу
    # Индексы для таблицы transactions
    Index(
        "idx_transactions_created_at", Transaction.created_at.desc()
    ),  # Сортировка по дате
    Index("idx_transactions_city_id", Transaction.city_id),  # Фильтрация по городу
    Index(
        "idx_transactions_type_id", Transaction.transaction_type_id
    ),  # Фильтрация по типу
    Index(
        "idx_transactions_specified_date", Transaction.specified_date.desc()
    ...
    Index(
        "idx_masters_login", Master.login
    ),  # Уникальный поиск по логину (уже есть unique, но для оптимизации)
    Index("idx_masters_phone", Master.phone_number),  # Поиск по телефону
    Index(
        "idx_masters_created_at", Master.created_at.desc()
    ),  # Сортировка по дате создания
    # Составные индексы для masters
    Index(
        "idx_masters_city_status", Master.city_id, Master.status
    ),  # Активные мастера по городам
    ...
    Index(
        "idx_employees_created_at", Employee.created_at.desc()
    ),  # Сортировка по дате создания
    # Составные индексы для employees
    Index(
        "idx_employees_role_city", Employee.role_id, Employee.city_id
    ),  # Сотрудники по роли и городу
    Index(
        "idx_employees_city_status", Employee.city_id, Employee.status
    ),  # Активные сотрудники по городам
    ...
    Index(
        "idx_administrators_login", Administrator.login
    ),  # Уникальный поиск по логину
    Index(
        "idx_administrators_created_at", Administrator.created_at.desc()
    ),  # Сортировка по дате создания
    ...
    Index(
        "idx_files_uploaded_at", File.uploaded_at.desc()
    ),  # Сортировка по дате загрузки
    # Составные индексы для files
    Index(
        "idx_files_request_type", File.request_id, File.file_type
    ),  # Файлы заявки по типу
    Index(
        "idx_files_transaction_type", File.transaction_id, File.file_type
    ),  # Файлы транзакции по типу
]
...
PERFORMANCE_INDEXES = [
    ...
    Index(
        "idx_requests_phone_time_window",
        Request.client_phone,
        Request.created_at.desc(),
    ),
    # Индекс для отчета колл-центра
    Index(
        "idx_requests_callcenter_report",
        Request.city_id,
        Request.status,
        Request.created_at.desc(),
    ),
    # Индекс для поиска записей звонков
    Index("idx_requests_phone_datetime", Request.client_phone, Request.created_at),
    # Индекс для финансовых отчетов
    Index(
        "idx_transactions_financial_report",
        Transaction.city_id,
        Transaction.specified_date.desc(),
        Transaction.amount,
    ),
        ...
        try:
            ...
            for index in DATABASE_INDEXES:
                try:
                    ...
                    logger.warning(
                        f"Index {index.name} already exists or failed to create: {e}"
                    )
            ...
            for index in PERFORMANCE_INDEXES:
                try:
                    ...
                    logger.warning(
                        f"Performance index {index.name} already exists or failed to create: {e}"
                    )
        ...
        try:
            ...
            for index in reversed(PERFORMANCE_INDEXES):
                try:
                    ...
                    logger.warning(
                        f"Performance index {index.name} doesn't exist or failed to drop: {e}"
                    )
            ...
            for index in reversed(DATABASE_INDEXES):
                try:
                    ...
                    logger.warning(
                        f"Index {index.name} doesn't exist or failed to drop: {e}"
                    )
    ...
    async with engine.begin() as conn:
        try:
            ...
            for row in slow_queries.fetchall():
                logger.info(
                    f"Query: {row[0][:100]}... | Calls: {row[1]} | Mean time: {row[2]:.2f}ms"
                )
            ...
            for row in index_usage.fetchall():
                logger.info(
                    f"Table: {row[1]} | Index: {row[2]} | Scans: {row[3]} | Tuples read: {row[4]}"
                )

        except Exception as e:
            logger.warning(
                f"Query performance analysis failed (pg_stat_statements might not be enabled): {e}"
            )
