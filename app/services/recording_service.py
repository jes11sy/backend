                ...
                try:
                    for recording_info in recordings:
                        ...
                        if request:
                            linked_count += 1
                            logging.info(
                                f"RECORDING SERVICE: Linked {recording_info['filename']} to request {request.id}"
                            )
                        else:
                            logging.warning(
                                f"RECORDING SERVICE: Could not link {recording_info['filename']}"
                            )
        ...
        try:
            ...
            if recordings:
                ...
                async for db in get_db():
                    try:
                        for recording_info in recordings:
                            request = await link_recording_to_request(
                                db, recording_info
                            )
                            if request:
                                linked_count += 1
                        ...
                        logging.error(
                            f"RECORDING SERVICE: Manual download DB error: {e}"
                        )
            ...
            logging.info(
                f"RECORDING SERVICE: Manual download completed - {len(recordings)} downloaded, {linked_count} linked"
            )
            return result
...
async def start_recording_service():
    ...
    if settings.RAMBLER_IMAP_USERNAME and settings.RAMBLER_IMAP_PASSWORD:
        ...
        logging.warning(
            "RECORDING SERVICE: Not started - email credentials not configured"
        )
