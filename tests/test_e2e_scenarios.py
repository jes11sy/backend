...
class TestE2EFullRequestLifecycle:
    ...
    async def test_complete_request_workflow(
        ...
        response = await authenticated_client.patch(
            f"/api/v1/requests/{request_id}/assign-master", json=master_assignment
        )
        ...
        response = await authenticated_client.patch(
            f"/api/v1/requests/{request_id}", json=status_update
        )
        assert response.status_code == 200
        ...
        response = await authenticated_client.post(
            f"/api/v1/requests/{request_id}/files", files=files
        )
        assert response.status_code == 201
        ...
        response = await authenticated_client.patch(
            f"/api/v1/requests/{request_id}", json=completion_data
        )
        assert response.status_code == 200
...
class TestE2EUserManagement:
    ...
    async def test_user_registration_and_authentication_flow(
        ...
        # 2. Аутентификация
        login_data = {"login": "newuser", "password": "securepassword123"}
...
class TestE2EReportingAndAnalytics:
    ...
    async def test_comprehensive_reporting_flow(
        self, authenticated_client: AsyncClient, test_db: AsyncSession
    ):
...
class TestE2ESystemIntegration:
    ...
    async def test_external_services_integration(
        ...
        with patch("app.services.email_client.EmailClient.send_email") as mock_email:
            ...
            response = await authenticated_client.post(
                "/api/v1/notifications/email", json=email_data
            )
            assert response.status_code == 200
        ...
        with patch(
            "app.services.recording_service.RecordingService.save_recording"
        ) as mock_recording:
            ...
            response = await authenticated_client.post(
                "/api/v1/recordings/", json=recording_data
            )
            assert response.status_code == 201
