...
class APIContract:
    ...
    def validate_response(
        self, method: str, path: str, response_data: Any, status_code: int
    ) -> bool:
...
class RequestManagementContract:
    ...
    def get_contract() -> APIContract:
        ...
        requests_list_schema = {
            "type": "object",
            "properties": {
                "items": {"type": "array", "items": request_response_schema},
                "total": {"type": "integer"},
                "page": {"type": "integer"},
        ...
        contract.add_endpoint(
            "DELETE", "/api/v1/requests/{id}", status_codes=[200, 401, 404]
        )
    ...
    def get_contract() -> APIContract:
        ...
        contract.add_endpoint(
            ...
            response_schema={
                "type": "object",
                "properties": {
                    "items": {"type": "array", "items": transaction_response_schema},
                    "total": {"type": "integer"},
                },
                "required": ["items", "total"],
            },
...
class AuthenticationContract:
    ...
    def get_contract() -> APIContract:
        ...
        contract.add_endpoint("POST", "/api/v1/auth/logout", status_codes=[200, 401])
...
class ContractTestRunner:
    ...
    async def test_contract_compliance(
        self, client: AsyncClient, contract_name: str
    ) -> Dict[str, Any]:
    ...
    async def _test_endpoint(
        self, client: AsyncClient, contract: APIContract, endpoint_config: Dict
    ) -> Dict[str, Any]:
            ...
            if response.status_code < 400 and endpoint_config.get("response_schema"):
                try:
                    ...
                    contract.validate_response(
                        method, path, response_data, response.status_code
                    )
                except Exception as e:
    ...
    def _generate_test_data(self, schema: Optional[Dict]) -> Optional[Dict]:
        ...
        for prop_name, prop_schema in properties.items():
            if prop_name in required or prop_name in [
                "client_name",
                "description",
                "amount",
            ]:
...
class TestContractCompliance:
    ...
    async def authenticated_client(self, async_client: AsyncClient):
        ...
        with patch("app.core.auth.get_current_user") as mock_auth:
            mock_auth.return_value = {"id": 1, "login": "testuser", "role": "admin"}
            yield async_client

    @pytest.mark.asyncio
    async def test_requests_api_contract(
        self, contract_runner: ContractTestRunner, authenticated_client: AsyncClient
    ):
    ...
    @pytest.mark.asyncio
    async def test_transactions_api_contract(
        self, contract_runner: ContractTestRunner, authenticated_client: AsyncClient
    ):
    ...
    @pytest.mark.asyncio
    async def test_auth_api_contract(
        self, contract_runner: ContractTestRunner, async_client: AsyncClient
    ):
        """Тест соответствия API аутентификации контракту"""
        results = await contract_runner.test_contract_compliance(async_client, "auth")
...
class TestContractEvolution:
    ...
    async def test_new_optional_fields(self, authenticated_client: AsyncClient):
        ...
        response = await authenticated_client.post(
            "/api/v1/requests/", json=request_with_new_field
        )
    ...
    async def test_deprecated_fields_handling(self, authenticated_client: AsyncClient):
        ...
        response = await authenticated_client.post(
            "/api/v1/requests/", json=request_with_deprecated
        )
...
class TestContractValidation:
    ...
    def test_response_schema_validation(self):
        ...
        assert contract.validate_response(
            "POST", "/api/v1/requests/", valid_response, 201
        )
        ...
        with pytest.raises(ValueError):
            contract.validate_response(
                "POST", "/api/v1/requests/", invalid_response, 201
            )
