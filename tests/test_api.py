...
class TestRequestsAPI:
    ...
    async def test_create_request(self, client: TestClient, db_session: AsyncSession):
        ...
        login_response = client.post(
            "/api/v1/auth/login", json={"login": "test_employee", "password": password}
        )
...
class TestTransactionsAPI:
    ...
    async def test_create_transaction(
        ...
        # Авторизуемся
        login_response = client.post(
            "/api/v1/auth/login", json={"login": "test_manager", "password": password}
        )
        assert login_response.status_code == 200
...
class TestUsersAPI:
    ...
    async def test_create_master(self, client: TestClient, db_session: AsyncSession):
        ...
        # Авторизуемся
        login_response = client.post(
            "/api/v1/auth/login", json={"login": "test_admin", "password": password}
        )
        assert login_response.status_code == 200
...
class TestMetricsAPI:
    ...
    async def test_get_metrics_authorized(
        self, client: TestClient, db_session: AsyncSession
    ):
...
class TestErrorHandling:
    ...
    async def test_422_validation_error(self, client: TestClient):
        ...
        response = client.post(
            "/api/v1/auth/login",
            json={"login": "", "password": "password"},  # Пустой логин
        )
