...
class TestE2EUserManagement:
    ...
    async def test_complete_user_workflow(
        self, client: TestClient, db_session: AsyncSession, admin_token: str
    ):
        ...
        response = client.get(
            "/api/v1/masters", headers={"Authorization": f"Bearer {admin_token}"}
        )
        ...
        response = client.patch(
            f"/api/v1/masters/{master_id}/deactivate",
            headers={"Authorization": f"Bearer {admin_token}"},
        )
        assert response.status_code == 200
...
class TestE2ERequestManagement:
    ...
    @pytest.mark.asyncio
    async def test_complete_request_workflow(
        self, client: TestClient, db_session: AsyncSession, admin_token: str
    ):
        ...
        response = client.patch(
            f"/api/v1/requests/{request_id}/notes",
            json=notes_data,
            headers={"Authorization": f"Bearer {admin_token}"},
        )
        assert response.status_code == 200
        ...
        response = client.patch(
            f"/api/v1/requests/{request_id}/financial",
            json=financial_data,
            headers={"Authorization": f"Bearer {admin_token}"},
        )
        assert response.status_code == 200
        ...
        response = client.get(
            "/api/v1/requests/stats", headers={"Authorization": f"Bearer {admin_token}"}
        )
        assert response.status_code == 200
        stats = response.json()
...
class TestE2ETransactionManagement:
    ...
    async def test_complete_transaction_workflow(
        ...
        response = client.post(
            "/api/v1/transaction_types",
            json=transaction_type_data,
            headers={"Authorization": f"Bearer {admin_token}"},
        )
        ...
        update_data = {"amount": "2000.00", "notes": "Обновленная транзакция"}
        response = client.put(
            f"/api/v1/transactions/{transaction_id}",
            json=update_data,
            headers={"Authorization": f"Bearer {admin_token}"},
        ...
        response = client.get(
            "/api/v1/transactions", headers={"Authorization": f"Bearer {admin_token}"}
        )
        assert response.status_code == 200
        ...
        response = client.get(
            f"/api/v1/transactions?city_id={city_id}",
            headers={"Authorization": f"Bearer {admin_token}"},
        )
        assert response.status_code == 200
...
class TestE2ESecurityAndAuth:
    ...
    def test_rate_limiting(self, client: TestClient):
        ...
        assert any(
            status == 429 for status in responses[-10:]
        )  # В последних 10 запросах
...
class TestE2EDataIntegrity:
    ...
    async def test_data_consistency(
        ...
        response = client.get(
            f"/api/v1/masters?city_id={city_id}",
            headers={"Authorization": f"Bearer {admin_token}"},
        )
        assert response.status_code == 200
        masters = response.json()
