...
class TestUserAuthentication:
    ...
    async def test_authenticate_master(self, db_session: AsyncSession):
        ...
        authenticated_user = await authenticate_user(
            "test_master", password, db_session
        )
        ...
        wrong_auth = await authenticate_user(
            "test_master", "wrong_password", db_session
        )
    ...
    async def test_authenticate_employee(self, db_session: AsyncSession):
        ...
        # Тестируем аутентификацию
        authenticated_user = await authenticate_user(
            "test_employee", password, db_session
        )
        assert authenticated_user is not None
...
class TestAuthAPI:
    ...
    async def test_login_endpoint(self, client: TestClient, db_session: AsyncSession):
        ...
        response = client.post(
            "/api/v1/auth/login", json={"login": "test_master", "password": password}
        )
    ...
    async def test_login_wrong_credentials(self, client: TestClient):
        ...
        response = client.post(
            "/api/v1/auth/login",
            json={"login": "wrong_user", "password": "wrong_password"},
        )
