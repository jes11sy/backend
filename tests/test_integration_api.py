...
class TestAuthenticationFlow:
    ...
    async def test_complete_login_flow(
        self, client: TestClient, test_employee: Employee
    ):
        ...
        login_response = client.post(
            "/api/v1/auth/login",
            json={"login": test_employee.login, "password": "test_password"},
        ...
        protected_response = client.get(
            "/api/v1/requests/", headers={"Authorization": f"Bearer {token}"}
        )
    ...
    async def test_invalid_login_attempts(
        ...
        response = client.post(
            "/api/v1/auth/login",
            json={"login": test_employee.login, "password": "wrong_password"},
        )
        assert response.status_code == 401
        ...
        # Пустые данные
        response = client.post("/api/v1/auth/login", json={"login": "", "password": ""})
    ...
    async def test_token_expiration(self, client: TestClient, test_employee: Employee):
        ...
        response = client.get(
            "/api/v1/requests/", headers={"Authorization": f"Bearer {expired_token}"}
        )
        assert response.status_code == 401
...
class TestRequestsWorkflow:
    ...
    async def test_create_request_full_workflow(
        ...
        # 4. Обновление заявки
        update_data = {"status": "in_progress", "problem": "Обновленная проблема"}
    ...
    async def test_request_filtering_and_search(
        self, authenticated_client: TestClient, test_request: Request
    ):
        """Тест фильтрации и поиска заявок"""
        # Поиск по телефону
        response = authenticated_client.get(
            f"/api/v1/requests/?phone={test_request.client_phone}"
        )
        assert response.status_code == 200
        ...
        response = authenticated_client.get(
            f"/api/v1/requests/?status={test_request.status}"
        )
        assert response.status_code == 200
        requests = response.json()
        assert len(requests) > 0
        ...
        response = authenticated_client.get(
            f"/api/v1/requests/?city_id={test_request.city_id}"
        )
        assert response.status_code == 200
        requests = response.json()
        assert len(requests) > 0
        ...
        response = authenticated_client.get(
            f"/api/v1/requests/?master_id={test_request.master_id}"
        )
        assert response.status_code == 200
        requests = response.json()
        assert len(requests) > 0
    ...
    async def test_request_permissions(
        ...
        if test_request.master_id == test_master.id:
            response = master_client.put(
                f"/api/v1/requests/{test_request.id}", json={"status": "completed"}
            )
            assert response.status_code == 200
        else:
            response = master_client.put(
...
class TestTransactionsWorkflow:
    ...
    async def test_create_transaction_workflow(
        ...
        update_data = {"amount": 1500.00, "description": "Обновленная транзакция"}

        response = authenticated_client.put(
            f"/api/v1/transactions/{transaction_id}", json=update_data
        )
        assert response.status_code == 200
    ...
    async def test_transaction_filtering(
        ...
        # Фильтрация по типу
        response = authenticated_client.get(
            f"/api/v1/transactions/?type_id={test_transaction.transaction_type_id}"
        )
        assert response.status_code == 200
        ...
        # Фильтрация по мастеру
        response = authenticated_client.get(
            f"/api/v1/transactions/?master_id={test_transaction.master_id}"
        )
        assert response.status_code == 200
        transactions = response.json()
        assert len(transactions) > 0
        ...
        # Фильтрация по дате
        response = authenticated_client.get(
            f"/api/v1/transactions/?date={test_transaction.specified_date}"
        )
        assert response.status_code == 200
        transactions = response.json()
        assert len(transactions) > 0
...
class TestFileUploadWorkflow:
    ...
    async def test_file_upload_workflow(
        ...
        response = authenticated_client.post(
            f"/api/v1/files/upload-expense-receipt/{test_request.id}", files=files
        )
        assert response.status_code == 200
        ...
        response = authenticated_client.post(
            f"/api/v1/files/upload-bso/{test_request.id}", files=files
        )
        assert response.status_code == 200

    @patch("app.utils.file_security.validate_file_content")
    async def test_file_security_validation(
        self, mock_validate, authenticated_client: TestClient, test_request: Request
    ):
...
class TestErrorHandling:
    ...
    async def test_not_found_errors(self, authenticated_client: TestClient):
        ...
        response = authenticated_client.put(
            "/api/v1/requests/99999", json={"status": "completed"}
        )
        assert response.status_code == 404
...
class TestDatabaseIntegration:
    ...
    async def test_database_transactions(
        ...
        # Обновляем заявку
        response = authenticated_client.put(
            f"/api/v1/requests/{request_id}", json={"status": "completed"}
        )
        assert response.status_code == 200
