...
class TestAuthenticationExtended:
    ...
    async def test_authenticate_wrong_password(
        self, db_session: AsyncSession, test_employee
    ):
        """Тест аутентификации с неверным паролем"""
        user = await authenticate_user(
            test_employee.login, "wrong_password", db_session
        )
...
class TestAPIEndpointsExtended:
    ...
    async def test_error_handling_coverage(self, authenticated_client: TestClient):
        ...
        response = authenticated_client.post(
            "/api/v1/requests/", json={"invalid_field": "invalid_value"}
        )
...
class TestDatabaseOperations:
    ...
    async def test_model_relationships(
        self, db_session: AsyncSession, test_city, test_request_type, test_master
    ):
