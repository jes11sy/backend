...
class TestPerformanceBasics:
    ...
    def test_app_startup_time(self):
        ...
        assert (
            startup_time < 10.0
        ), f"App startup took {startup_time:.2f}s, expected < 10s"
    ...
    def test_health_check_response_time(self):
        ...
        with TestClient(app) as client:
            ...
            assert (
                response_time < 2.0
            ), f"Health check took {response_time:.2f}s, expected < 2s"
    ...
    def test_openapi_schema_response_time(self):
        ...
        with TestClient(app) as client:
            ...
            assert (
                response_time < 1.0
            ), f"OpenAPI schema took {response_time:.2f}s, expected < 1s"
            assert response.status_code == 200
...
class TestMemoryUsage:
    ...
    def test_memory_leak_detection(self):
        ...
        for i in range(1, len(memory_measurements)):
            ...
            assert (
                memory_diff < 50
            ), f"Memory increased by {memory_diff:.2f}MB between measurements"
...
class TestResponseSizes:
    ...
    def test_docs_response_size(self):
        ...
        with TestClient(app) as client:
            ...
            ), f"Docs size is {content_size} bytes, expected < 5MB"
    ...
    def test_openapi_schema_size(self):
        ...
        with TestClient(app) as client:
            ...
            content_size = len(response.content)
            assert (
                content_size < 1 * 1024 * 1024
            ), f"OpenAPI schema size is {content_size} bytes, expected < 1MB"
...
class TestStressTests:
    ...
    def test_rapid_requests_stress(self):
        ...
        with TestClient(app) as client:
            ...
            assert (
                success_rate > 0.8
            ), f"Success rate is {success_rate:.2%}, expected > 80%"

            # Средняя скорость должна быть разумной
            avg_time_per_request = total_time / 100
            assert (
                avg_time_per_request < 0.5
            ), f"Average time per request: {avg_time_per_request:.3f}s"
    ...
    def test_endpoint_availability_under_load(self):
        ...
        with ThreadPoolExecutor(max_workers=len(endpoints)) as executor:
            futures = [
                executor.submit(test_endpoint, endpoint) for endpoint in endpoints
            ]
            results = [future.result() for future in futures]
        ...
        for i, success_count in enumerate(results):
            ...
            assert (
                success_rate > 0.7
            ), f"Endpoint {endpoints[i]} success rate: {success_rate:.2%}"
