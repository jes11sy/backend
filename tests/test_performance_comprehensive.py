...
class TestAPIPerformance:
    ...
    async def test_authentication_performance(self, client: TestClient, test_employee):
        ...
        login_data = {"login": test_employee.login, "password": "test_password"}
...
class TestConcurrencyPerformance:
    ...
    async def test_concurrent_requests(self, client: TestClient):
        ...
        print(
            f"Concurrent requests (20) - Success: {success_count}/20, Time: {total_time:.3f}s"
        )
    ...
    async def test_concurrent_authentication(self, client: TestClient, test_employee):
        ...
        print(
            f"Concurrent auth (10) - Success: {success_count}/10, Time: {total_time:.3f}s"
        )
    ...
    async def test_concurrent_database_operations(
        ...
        print(
            f"Concurrent DB ops (10) - Success: {success_count}/10, Time: {total_time:.3f}s"
        )
...
class TestMemoryPerformance:
    ...
    async def test_memory_usage_normal_operations(
...
class TestCachePerformance:
    ...
    async def test_cache_hit_performance(
        self, mock_cache, authenticated_client: TestClient
    ):
...
class TestLoadTesting:
    ...
    async def test_sustained_load(self, client: TestClient):
        ...
        print(
            f"Sustained load test - Success rate: {success_rate:.1f}%, Time: {total_time:.1f}s"
        )
    ...
    async def test_spike_load(self, client: TestClient):
        ...
        print(
            f"Spike load test - Success rate: {success_rate:.1f}%, Time: {total_time:.1f}s"
        )
...
class TestPerformanceRegression:
    ...
    async def test_performance_baseline(self, authenticated_client: TestClient):
        ...
        operations = [
            ...
            (
                "cities_list",
                lambda: authenticated_client.get("/api/v1/requests/cities"),
            ),
            (
                "types_list",
                lambda: authenticated_client.get("/api/v1/requests/request-types"),
            ),
            (
                "masters_list",
                lambda: authenticated_client.get("/api/v1/requests/masters"),
            ),
        ]
        ...
        for operation_name, results in performance_results.items():
            print(
                f"  {operation_name}: avg={results['average']:.3f}s, max={results['maximum']:.3f}s"
            )
    ...
    async def test_database_performance_baseline(
        ...
        print(
            f"Database baseline - Insert: {insert_time:.3f}s, Simple: {simple_query_time:.3f}s, Complex: {complex_query_time:.3f}s"
        )
