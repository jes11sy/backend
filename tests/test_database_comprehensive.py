...
class TestCRUDOperations:
    ...
    async def test_get_requests_crud(
        self, db_session: AsyncSession, test_request: Request
    ):
    ...
    async def test_update_request_crud(
        ...
        update_data = {"status": "completed", "problem": "Обновленная проблема"}
...
class TestDatabaseConstraints:
    ...
    async def test_foreign_key_constraints(
        self, db_session: AsyncSession, test_request_type: RequestType
    ):
    ...
    async def test_not_null_constraints(
        self, db_session: AsyncSession, test_city: City, test_request_type: RequestType
    ):
    ...
    async def test_index_usage(
        ...
        result = await db_session.execute(
            select(Request).where(
                (Request.city_id == test_city.id) & (Request.status == "new")
            )
        )
...
class TestDatabaseMigrations:
    ...
    async def test_column_types(self, db_session: AsyncSession):
        ...
        result = await db_session.execute(text("PRAGMA table_info(requests)"))
