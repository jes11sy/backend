    ...
    async def make_request(
        ...
        try:
            async with self.session.request(
                method, f"{self.config.BASE_URL}{endpoint}", **kwargs
            ) as response:
            ...
            return {"status_code": 500, "response_time": response_time, "error": str(e)}
...
class TestMemoryAndResourceLoad:
    ...
    async def test_memory_leak_detection(self):
        ...
        for cycle in range(3):
            ...
            print(
                f"Cycle {cycle + 1}: Memory increase: {memory_increase / 1024 / 1024:.2f} MB"
            )
...
class TestRealisticUserScenarios:
    ...
    async def _simulate_master_user(self, runner: LoadTestRunner):
        ...
        await runner.make_request(
            "PATCH", "/api/v1/requests/1", json={"status": "in_progress"}
        )
        await asyncio.sleep(2)
