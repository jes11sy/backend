...
def status():
    ...
    try:
        ...
        click.echo(f"\n📊 Статус миграций:")
        click.echo(
            f"   Текущая ревизия: {status_data['current_revision'] or 'Не инициализирована'}"
        )
        click.echo(
            f"   Последняя ревизия: {status_data['head_revision'] or 'Не найдена'}"
        )
        click.echo(
            f"   Актуальность: {'✅ Актуальна' if status_data['is_up_to_date'] else '❌ Требуется обновление'}"
        )
        ...
        if status_data["history"]:
            ...
            for migration in status_data["history"][:5]:
                click.echo(
                    f"   - {migration['revision']}: {migration['doc'] or 'Без описания'}"
                )
...
def validate():
    ...
    try:
        ...
        if validation_result["valid"]:
            ...
            click.echo(
                f"   Таблица alembic_version: {'✅ Найдена' if validation_result['has_alembic_table'] else '❌ Не найдена'}"
            )
        else:
            ...
            if "missing_tables" in validation_result:
                click.echo(
                    f"   Отсутствующие таблицы: {', '.join(validation_result['missing_tables'])}"
                )
...
@click.option(
    ...
    default=True,
    help="Автоматическое создание миграции",
)
...
@cli.command()
@click.option(
    "--revision", default="head", help="Ревизия для применения (по умолчанию: head)"
)
@click.option(
    "--backup/--no-backup",
    default=True,
    help="Создать резервную копию перед применением",
)
...
@click.option(
    "--backup/--no-backup", default=True, help="Создать резервную копию перед откатом"
)
...
def history(limit: int):
    ...
    try:
        ...
        for i, migration in enumerate(history_data[:limit], 1):
            ...
            click.echo(
                f"      Родительская: {migration['down_revision'] or 'Корневая'}"
            )
            click.echo(
                f"      Файл: {os.path.basename(migration['path']) if migration['path'] else 'Не найден'}"
            )
            click.echo()
...
@click.option(
    ...
    default=False,
    help="Автоматически применить миграции",
)
