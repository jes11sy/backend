...
def main():
    ...
    if not run_command([sys.executable, "-m", "pytest", "--version"], "Проверка pytest"):
        print("❌ pytest не установлен. Устанавливаю...")
        if not run_command([sys.executable, "-m", "pip", "install", "pytest", "pytest-cov", "pytest-asyncio"], "Установка pytest"):
