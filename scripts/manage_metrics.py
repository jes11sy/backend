...
from app.metrics import metrics_collector, performance_collector, business_collector
...
@click.option(
    "--limit", "-l", type=int, default=10, help="Максимальное количество значений"
)
@click.option(
    "--format",
    "-f",
...
def show_metric(metric_name, since, limit, format):
    ...
    for value in values:
        tags_str = (
            ", ".join([f"{k}={v}" for k, v in value.tags.items()])
            if value.tags
            else "-"
        )
        metadata_str = (
            ", ".join([f"{k}={v}" for k, v in value.metadata.items()])
            if value.metadata
            else "-"
        )
...
@click.argument("metric_name")
@click.option(
    "--since", "-s", help="Показать статистику с определенной даты (YYYY-MM-DD)"
)
...
def business_summary(format):
    ...
    business_metrics = {
        ...
        "Общее количество транзакций": metrics_collector.get_latest_value(
            "transactions_total"
        ),
        ...
        "Среднее время обработки (сек)": metrics_collector.get_latest_value(
            "avg_processing_time"
        ),
        ...
        "Средняя длительность звонка (сек)": metrics_collector.get_latest_value(
            "call_duration_avg"
        ),
    }
...
def performance_summary(format):
    ...
    performance_metrics = {
        "HTTP запросы (всего)": metrics_collector.get_latest_value(
            "http_requests_total"
        ),
        ...
        "Активные DB соединения": metrics_collector.get_latest_value(
...
@click.option(
    ...
    type=int,
    default=24,
    help="Удалить метрики старше указанного количества часов",
)
...
@click.option(
    "--since", "-s", help="Экспортировать данные с определенной даты (YYYY-MM-DD)"
)
...
def dashboard():
    ...
    business_data = [
        ...
        [
            "Конверсия (%)",
            f"{metrics_collector.get_latest_value('conversion_rate') or 0:.1f}%",
        ],
        [
            "Дневная выручка",
            f"{metrics_collector.get_latest_value('revenue_daily') or 0:.2f} ₽",
        ],
        [
            "Активные пользователи",
            metrics_collector.get_latest_value("active_users") or 0,
        ],
        [
            "Транзакции (всего)",
            metrics_collector.get_latest_value("transactions_total") or 0,
        ],
    ...
    performance_data = [
        [
            "Память (MB)",
            f"{metrics_collector.get_latest_value('memory_usage') or 0:.1f}",
        ],
        ["CPU (%)", f"{metrics_collector.get_latest_value('cpu_usage') or 0:.1f}%"],
        [
            "HTTP запросы",
            metrics_collector.get_latest_value("http_requests_total") or 0,
        ],
