...
class ApiDemo:
    ...
    async def demo_health_check(self):
        ...
        if self.auth_token:
            ...
            response = await self.client.get(
                url, headers={"Authorization": f"Bearer {self.auth_token}"}
            )
            self.print_response(response)
    ...
    async def demo_authentication(self):
        ...
        credentials_examples = [
            {"name": "Мастер", "login": "master001", "password": "password123"},
            {
                "name": "Сотрудник колл-центра",
                "login": "callcenter_user",
                "password": "employee_pass456",
            },
            {"name": "Администратор", "login": "admin", "password": "admin_secure789"},
        ]

        for cred in credentials_examples:
            print(f"\n👤 Попытка входа: {cred['name']}")
            url = f"{self.base_url}/api/v1/auth/login"
            data = {"login": cred["login"], "password": cred["password"]}
            ...
            if response.status_code == 200:
                ...
                me_response = await self.client.get(
                    me_url, headers={"Authorization": f"Bearer {self.auth_token}"}
                )
    ...
    async def demo_requests_api(self):
        ...
        create_response = await self.client.post(
            create_url, json=full_request_data, headers=headers
        )
        ...
        minimal_response = await self.client.post(
            create_url, json=minimal_request_data, headers=headers
        )
    ...
    async def demo_transactions_api(self):
        ...
        create_response = await self.client.post(
            create_url, json=expense_data, headers=headers
        )
        self.print_response(create_response)
        ...
        salary_response = await self.client.post(
            create_url, json=salary_data, headers=headers
        )
    ...
    async def demo_users_api(self):
        ...
        create_master_response = await self.client.post(
            create_master_url, json=master_data, headers=headers
        )
    ...
    async def demo_error_handling(self):
        ...
        wrong_data = {"login": "wrong_user", "password": "wrong_password"}
        ...
        if self.auth_token:
            ...
            invalid_response = await self.client.post(
                create_url, json=invalid_data, headers=headers
            )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Демонстрация API системы управления заявками"
    )
    parser.add_argument(
        "--url", default="http://localhost:8000", help="Базовый URL API"
    )
    parser.add_argument(
        "--section",
        choices=[
        ...
        default="all",
        help="Какую секцию демонстрировать",
    )
